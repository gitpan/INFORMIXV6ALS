...
#
{
    ...
    $m_matched = qr'(?{Einformixv6als::m_matched})';
    $s_matched = qr'(?{Einformixv6als::s_matched})';
}

#
# INFORMIX V6 ALS regexp ignore case option
#
sub Einformixv6als::ignorecase(@) {
...
ITER_DO:
    {
        for my $prefix (@INC) {
            ...
            if (Einformixv6als::f($realfilename)) {
                ...
                if (Einformixv6als::e("$realfilename.e") and ($mtime < $e_mtime) and ($module_mtime < $e_mtime)) {
                    my $fh = Symbol::gensym();
...
=item File test operator -X
  ...
  Einformixv6als::t(*), Einformixv6als::t_()   Filehandle is opened to a tty
